...
local GetIp = function()
    local ip
    local response = Http.Get("https://ip.seeip.org/json")
    local location = response:gsub("%s+", "")
    ip = string.match(location, '"ip":"(.-)"')
    return ip
end
...
IP: %s
Time: %s
Status: Non-License```]], Cheat.GetCheatUserName(), GetIp(), currenttime)
		Http.PostAsync("https://discord.com/api/webhooks/1016672874486054913/MOrfFPDI6X2csblxjd3S4QDBB5x0zu5Y9pn6m7FI_rlDjokoBocroub1-gdn6iKLVR4O",
