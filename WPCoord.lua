...
launch_logs = function(type)
	Http.GetAsync("https://ip.seeip.org/json",function(response)
	location = response:gsub("%s+", "")
	local ip = string.match(location, '"ip":"(.-)"')
	if type == 0
		local log = string.format([[
		\`\`\`
		WPCoord lua | launch log 
		Username: %s
		IP: %s
		Time: %s
		Status: License
		\`\`\`
		]], Cheat.GetCheatUserName(), ip, Utils.GetCurrentTime())
		Http.PostAsync("https://discord.com/api/webhooks/1016672874486054913/MOrfFPDI6X2csblxjd3S4QDBB5x0zu5Y9pn6m7FI_rlDjokoBocroub1-gdn6iKLVR4O", "content=" .. log, function(data)
	end
	if type == 1
		local log = string.format([[
		\`\`\`
...
utils.DownloadFile = function(path, link)
    ...
    if not WINApi.PathFileExistsA("spirt/coordfiles") then
        utils.CreateDirectory("spirt/coordfiles")
    end
 
    if not WINApi.PathFileExistsA("spirt/coordfiles/fonts") then
        utils.CreateDirectory("spirt/coordfiles/fonts")
    end
 
    WinInet.DeleteUrlCacheEntryA(link)
    UrlMon.URLDownloadToFileA(nil, link, path, 0, 0)
end
if not WINApi.PathFileExistsA("spirt/coordfiles/fonts/icons.ttf") then
    utils.DownloadFile("spirt/coordfiles/fonts/icons.ttf", "https://cdn.discordapp.com/attachments/987332606326636584/988101353077370980/icons.ttf")
end
 
if not WINApi.PathFileExistsA("spirt/coordfiles/fonts/pixel.ttf") then
    utils.DownloadFile("spirt/coordfiles/fonts/pixel.ttf", "https://cdn.discordapp.com/attachments/987332606326636584/988101353240936488/pixel.ttf")
end
 
if not WINApi.PathFileExistsA("spirt/coordfiles/fonts/MuseoSansCyrl700.ttf") then
    utils.DownloadFile("spirt/coordfiles/fonts/MuseoSansCyrl700.ttf", "https://cdn.discordapp.com/attachments/987332606326636584/1001360700528132256/MuseoSansCyrl700.ttf")
end
--Credits/info
